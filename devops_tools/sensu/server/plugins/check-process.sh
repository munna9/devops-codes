...
# usage
HELP="
    usage: $0 [ -p value -f value -r value -h ]
        -p --> process name
        -f --> file path to pid file
		-r --> port number where process is running
...
if [ ${PORT} ]; then
  ret=`lsof -i tcp:"${PORT}" | grep java | awk '{print $2}'|sort | uniq`
  if [ ! "${ret}" ]; then
    echo "CRITICAL - process is not running on port ${PORT}"
    exit 2
  fi
  echo "PROCESS OK on - ${PORT}"
  exit 0
fi
