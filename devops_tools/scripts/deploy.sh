...
function copy()
{
	...
	ssh -o "StrictHostKeyChecking no" -i ${DHOME}/${accessKey} -tt ${user}@${serverName} " if [ -d $destDir ]; then echo "dest dir exist"; else mkdir -p $destDir; fi"
	scp -o "StrictHostKeyChecking no" -i ${DHOME}/${accessKey} ${build_number}/${srcFile} ${user}@${serverName}:${destFile}
	if [ $? -eq 0 ]
	then
...
{
	set -e
	echo "		--TOKENIZE_FILE function to find replace given strings in a file **********"
	input=$1
	fileName=`echo $input | awk -F"##" '{print $2}' | sed 's/^ *//;s/ *$//'`
	findString=`echo $input | awk -F"##" '{print $3}' | sed 's/^ *//;s/ *$//'`
	replaceString=`echo $input | awk -F"##" '{print $4}' | sed 's/^ *//;s/ *$//'`
	absolutePath=
		echo "		-- The file $fileName exist and performing the localize"
		sed -i "s%$findString%$replaceString%g" $fileName
	set +e
...
function run()
{
	...
	ssh -o "StrictHostKeyChecking no" -i ${DHOME}/${accessKey} -tt ${user}@${serverName} "${command}"
	if [ $? -eq 0 ]
	then
...
function copylocalfile()
{
	...
	echo "		--Executing the given commands  <cp -r ${DHOME}/${filelocation1} ${filelocation2}/> on local machine "
	
	cp -r ${DHOME}/${filelocation1} ${filelocation2}/
	if [ $? -eq 0 ]
	then
		echo "		-- The  <cp -r ${DHOME}/${filelocation1} ${filelocation2}/> has been successfully completed"
	else
...
function servercopy()
{
	...
	ssh -o "StrictHostKeyChecking no" -i ${DHOME}/${accessKey} -tt ${user}@${serverName} " if [ -d $destDir ]; then echo "dest dir exist"; else mkdir -p $destDir; fi"
	scp -o "StrictHostKeyChecking no" -i ${DHOME}/${accessKey} ${srcFile} ${user}@${serverName}:${destFile}
	if [ $? -eq 0 ]
	then
		echo "		--The copying artifact $srcFile has been successfully completed ********"
	else
	...
	do {
		...
		task=`echo $task | sed 's/^ *//;s/ *$//'`
			...
			case $task in
				...
				tokenize)	echo "!!!!****** Calling tokenize Method ******!!!!" 
							tokenize "$line";;			
