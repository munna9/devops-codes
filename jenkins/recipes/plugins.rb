directory node['jenkins']['plugin']['home_directory'] do
...
node['jenkins']['plugin']['packages'].each do |plugin_name,plugin_version|
  if node['jenkins']['plugin']['fetch_latest']
    remote_file "#{node['jenkins']['plugin']['home_directory']}/#{plugin_name}.hpi" do
      source "#{node['jenkins']['plugin']['permalink']}/#{plugin_name}.hpi"
      owner node['jenkins']['service']['owner']
      group node['jenkins']['service']['group']
      mode '0644'
      sensitive true
