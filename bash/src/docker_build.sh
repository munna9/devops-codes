...
WHATPUSHED=$(git whatchanged -1 --format=%f $REPO_BASE_DIRECTORY | grep '^:' | cut -f5 -d ' ' | sed 's/^[AMD]\t*//g' | cut -f1 -d'/' | sort -u)
for EACH_DIRECTORY in $WHATPUSHED; do
    if [ $EACH_DIRECTORY == 'docker' ]; then
